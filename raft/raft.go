...
const Election_Timeout_Max time.Duration = 3 * time.Second
const (
	Leader ServerState = 1 << iota
	Follower
	Candidate
)
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term > rf.currentTerm || (args.Term == rf.currentTerm && rf.votedFor == nil) {
		rf.state = Follower
			...
			for i := range rf.peers {
				...
				go func (index int)  {
					...
					if res.Term > rf.currentTerm {
						log.Printf("Node %v downgraded to Follower because it has smaller term than one of the leader: %v < %v", rf.me, rf.currentTerm, res.Term)
...
func (rf *Raft) ticker() {
	for !rf.killed() {
		...
		if time.Since(rf.electionTimer) > Election_Timeout_Max && rf.state != Leader {
			...
			for rf.state == Candidate {
				...
				for i := range rf.peers {
					...
					go func (index int)  {
						...
						succeeded := rf.sendRequestVote(index, &req, &res)
						...
						if res.Term > rf.currentTerm {
							log.Printf("Node %v downgraded to Follower because it has smaller term than one of the candidate: %v < %v", rf.me, rf.currentTerm, res.Term)
				...
				if int(atomic.LoadInt32(&voteGranted)) > len(rf.peers) / 2 {
					...
					log.Printf("New leader selected, new leader ID: %v, term: %v", rf.me, rf.currentTerm)
				} else {
					log.Printf("Node %v: Failed to elect a leader, waiting for the next time window...", rf.me)
				}
